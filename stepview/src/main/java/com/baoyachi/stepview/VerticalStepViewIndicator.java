...
public class VerticalStepViewIndicator extends View
{
    ...
    //定义默认的高度   definition defaulticon height
    private int defaultStepIndicatorNum = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 35, getResources().getDisplayMetrics());
    ...
    private Drawable mCompleteIcon;//完成的默认图片    definition defaulticon completed icon
    private Drawable mAttentionIcon;//正在进行的默认图片     definition defaulticon underway icon
    private Drawable mDefaultIcon;//默认的背景图  definition defaulticon unCompleted icon
    ...
    private void init()
    {
        ...
        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable.right);//已经完成的icon
        mAttentionIcon = ContextCompat.getDrawable(getContext(), R.drawable.oval);//正在进行的icon
        mDefaultIcon = ContextCompat.getDrawable(getContext(), R.drawable.defaulticon);//未完成的icon

        mIsReverseDraw = true;//defaulticon draw
    }
