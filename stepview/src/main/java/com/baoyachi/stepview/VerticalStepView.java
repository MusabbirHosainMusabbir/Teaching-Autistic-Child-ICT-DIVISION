...
public class VerticalStepView extends LinearLayout implements VerticalStepViewIndicator.OnDrawIndicatorListener
{
    ...
    private int mUnComplectedTextColor = ContextCompat.getColor(getContext(), R.color.textcolor);//定义默认未完成文字的颜色;
    private int mComplectedTextColor = ContextCompat.getColor(getContext(), R.color.textcolor);//定义默认完成文字的颜色;
    ...
    {
        Log.e("texts", String.valueOf(texts.size()));
        mTexts = texts;
        if(texts != null){
    ...
    public void ondrawIndicator()
    {
        if(mTextContainer != null)
        {
            ...
            {
                Log.e("mtextsize", String.valueOf(mTexts.size()));
                for(int i = 0; i < mTexts.size(); i++)
                {
                    Log.e("mtextxts",mTexts.get(i).getName().toString());
                    mTextView = new TextView(getContext());
                    mTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTextSize);
                    mTextView.setText(mTexts.get(i).getName().toString());
